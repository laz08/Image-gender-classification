...
def describe(numPoints, image, radius, eps=1e-7):
    ...
    (hist, edges) = np.histogram(lbp.ravel(),
        ...
        range=(0, numPoints + 2))
    #plt.hist(lbp.ravel(),256,[0,50]); plt.show()
...
def performKNeighbors(training, test):
    ...
    model = KNeighborsClassifier(n_neighbors=31, algorithm='brute')
    model.fit([item[2] for item in training], [item[1] for item in training])
    ...
    for i, ind in enumerate(mat):
        ...
        if os.path.exists(_PATH_TO_PHOTOS):
            ...
            if(image is None):
                ...
                #if(str(ind[0]) is str('Nicole_Kidman_0015.jpg')):
                ...
                #if(str(ind[0]) == 'Nicole_Kidman_0015.jpg'):
                #plotImage(resized)
               
                #gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                #hist = describe(24, resized, 8)
                hist = describe(nPoints, resized, rad)
                #plotHist1(hist)
                cv2.normalize(hist, hist)
                hist.flatten()
                data.append(hist)
